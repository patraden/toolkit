...
#
# Based on: https://github.com/vertica/vertica-containers/blob/main/one-node-ce/env_setup/vertica_env.sh
#
# Modifications from upstream:
#   - Removed `startdb` / `stopdb` aliases: on a multi-node cluster those need
#     `run_init` (root) on every peer to be safe. Use `make install-vertica` +
#     `admintools -t start_db/stop_db` from the leader instead.
